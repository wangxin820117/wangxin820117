...
class ActivePlanner:
    ...
    def __init__(
        ...
        grid_center: np.array = np.array([1.0, 0.0, 1.15]),
        ...
        num_samples: int = 10,
        save_path: str = None,
        target_params: np.array = np.array([1.0, 0.0, 1.15]),
    ) -> None:
        """
        ...
        self.active_vision_params(start_pose, target_params)
    ...
    def active_view(self, seed=None) -> np.array:
        """
        ...
        for i, view in enumerate(view_samples):
            with torch.no_grad():
                _, gain, _ = self.voxel_grid.compute_gain(
                    camera_poses[i, :3], camera_poses[i, 7:]
                )
        ...
        return self.viewpoint, 1.0 - best_gain, len(view_samples)
